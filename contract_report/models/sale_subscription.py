...
class sale_subscription_report_methods(models.Model):
    ...
    def get_timesheets(self):
        _logger.debug("\n\n%s" % self.env['account.analytic.line'].search([('is_timesheet', '=', True), ('account_id', '=', self.analytic_account_id.id)]))
        return self.env['account.analytic.line'].search([('is_timesheet', '=', True), ('account_id', '=', self.analytic_account_id.id)])

    @api.model
    def get_standalone(self):
        return self.env['account.analytic.line'].search([('is_timesheet', '=', True), ('account_id', '=', self.analytic_account_id.id), ('task_id', '=', False)])
    ...
    def check_start_end_line_date(self, lineDate):
        ...
        start_date = self.env['contract.report'].get_start_date()
        if not start_date:
            start_date = datetime.datetime.strptime("1980-01-01", "%Y-%m-%d").date()
        else:
            start_date = datetime.datetime.strptime(start_date, "%Y-%m-%d").date()

        end_date = self.env['contract.report'].get_end_date()
        if end_date:
    ...
    @api.model
    def isRemoveWorklogs(self):
        return self.env['contract.report'].get_only_worklogs()

    @api.model
    def isGroupByIssueTask(self):
