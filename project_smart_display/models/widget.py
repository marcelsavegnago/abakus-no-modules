...
class Widget(models.Model):
    ...
    def _check_graph_generated_without_error(self):
        try:
            self._handle_widget_graph(self, 'for exception')
        except Exception as e:
            raise exceptions.ValidationError(e) # display exception if a graph generate one
            
    def _handle_widget_graph(self, widget, smart_dashboard_html):
        if widget.filter_domain == False: # no domain
            domain = []
        else:
            domain = widget.filter_domain
                
        if widget.graph_type == 'sector':
            smart_dashboard_html = self._handle_graph_sector(smart_dashboard_html, widget, domain)
        elif widget.graph_type == 'bar':
            smart_dashboard_html = self._handle_graph_bar(smart_dashboard_html, widget, domain)
        elif widget.graph_type == 'linear':
            smart_dashboard_html = self._handle_graph_linear(smart_dashboard_html, widget)
        return smart_dashboard_html
    
    def _handle_graph_sector(self, smart_dashboard_html, widget, domain):
        result = self._generate_labels_and_values_for_graph_sector_and_bar(widget, domain)
        labels = result[0]
        values = result[1]
    ...
    def _handle_graph_bar(self, smart_dashboard_html, widget, domain):
        result = self._generate_labels_and_values_for_graph_sector_and_bar(widget, domain)
        labels = result[0]
        values = result[1]
    ...
    def _generate_labels_and_values_for_graph_sector_and_bar(self, widget, domain):
        labels = []
        values = []
        nb_occurence_for_label_other = 0
        field = widget.group_by_field
        field_values = self.env[widget.model].sudo().search([]).mapped(field.name)
    ...
    def _handle_graph_linear(self, smart_dashboard_html, widget):
        labels = []
        values = []
        result = []
    ...
    def _handle_graph_linear_operation(self, widget, start_date, end_date):
        values = request.env[widget.model].sudo().search([('create_date', '>=', start_date), ('create_date', '<=', end_date)]).mapped(widget.operation_field.name)
