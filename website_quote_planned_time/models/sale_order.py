...
class SaleOrder(models.Model):
    ...
    def onchange_template_id(self):
        ...
        if self.partner_id:
            ...
            context['lang'] = self.pool['res.partner'].browse(self.partner_id.id).lang

        pricelist_obj = self.env['product.pricelist']

        lines = [(5,)]
        quote_template = self.pool.get('sale.quote.template').browse(self.template_id.id)
        for line in quote_template.quote_line:
            ...
            if 'tax_id' in data:
                ...
                fpos = (self.fiscal_position_id and self.pool['account.fiscal.position'].browse(self.fiscal_position_id.id)) or False
