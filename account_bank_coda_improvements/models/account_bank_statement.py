...
class account_next_sequence(models.Model):
    ...
    def _compute_bank_account_balance(self):
        journal_id = self.env['account.journal'].search([('id', '=', self.journal_id.id)])
        if journal_id:
            self.bank_account_balance = (obj.browse(journal_id[0])).default_debit_account_id.compute_account_balance()
        else:
